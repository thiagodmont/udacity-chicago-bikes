...
for i, data in enumerate(data_list[:20]):
    print("L {} : gênero {}".format(i + 1, data[6]))
...
def column_to_list(data, index):
    ...
    """
    Responsável por adicionar as colunas(features) de uma lista em uma outra nova lista, seguindo a mesma ordem.
      Argumentos:
          data: list
          index: int
      Retorna:
...
def count_gender(data_list):
    """
    Função responsável por contar a quantidade de cada gênero.
      Argumentos:
          data_list: list
      Retorna:
          list [qntd_male, qntd_female].
    """
    male = 0
    female = 0
...
def most_popular_gender(data_list = []):
    """
    Responsável por retornar o gênero mais polular na lista.
      Argumentos:
          data_list: list
      Retorna:
          o gênero mais popular: string
    """
    gender = ""
...
quantity = [user_types.count("Customer"), user_types.count("Dependent"), user_types.count("Subscriber") ]
...
plt.bar(y_pos, quantity)
plt.ylabel('Quantity')
plt.xlabel('User\'s type')
plt.xticks(y_pos, types)
plt.title('Quantity vs User\'s type')
plt.show(block=True)
...
answer = "Na lista existem {} amostras sem informação de gênero.".format(empty)
...
def count_items(column_list):
    """
    Responsável por contar as ocorrências de item diferentes na lista, sem definição prévia.
      Argumentos:
          column_list: list
      Retorna:
          Tuple: (item_types, count_items):
    """
